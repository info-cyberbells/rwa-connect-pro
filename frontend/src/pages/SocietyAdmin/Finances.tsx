...
import {
  Wallet, Search, PlusCircle, Plus, History, Send, Eye, Edit2,
...
const GenerateBillsModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card text-[#0F172A] h-[90vh] w-full max-w-4xl flex flex-col rounded-[20px] shadow-2xl overflow-hidden">
        <div className="py-4 px-8 flex items-start justify-between border-b">
          ...
          <button onClick={onClose} className="hover:bg-slate-100 rounded-full text-muted-foreground"><X size={24} /></button>
        </div>
            ...
            <section className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Month</label>
                  <div className="relative">
                    <select value={formData.month} onChange={(e) => handleInputChange('month', e.target.value)}
                      className="w-full bg-muted/50 border border-border rounded-2xl px-4 py-3 text-sm appearance-none outline-none font-medium">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Year</label>
                  <div className="relative">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="date" value={formData.dueDate} onChange={(e) => handleInputChange('dueDate', e.target.value)}
                    className="w-full bg-muted/50 border border-border rounded-2xl px-4 py-3 text-sm outline-none font-medium" />
                </div>
              </div>
            </section>
                  ...
                  <div key={key} className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input type="number" value={formData[key]} onChange={(e) => handleInputChange(key, e.target.value)}
                        placeholder={placeholder} className="w-full bg-muted/50 border border-border rounded-2xl py-3 pl-8 pr-4 text-sm outline-none font-bold" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="space-y-4">
                ...
                {['all', 'wings', 'individual'].map((type) => (
                  <button key={type} onClick={() => handleInputChange('recipientType', type)}
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-2xl transition-all ${
                      formData.recipientType === type ? 'border border-[#3B82F6] bg-[#3B82F6]/10' : 'border border-gray-200 bg-muted/50'
                    }`}>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search and select units..."
                  className="w-full bg-muted/50 border border-border rounded-2xl py-3 pl-12 pr-4 text-sm outline-none" />
              </div>
            </section>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div onClick={() => handleInputChange('autoNotification', !formData.autoNotification)}
                    className={`w-12 h-6 rounded-full relative cursor-pointer transition-all ${formData.autoNotification ? 'bg-blue-400' : 'bg-card/10'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-card rounded-full shadow-sm transition-all ${formData.autoNotification ? 'right-1' : 'left-1'}`} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-end gap-4 py-4 px-8 border-t bg-card">
          <button onClick={onClose} className="px-8 py-2 text-sm font-bold text-muted-foreground border rounded-2xl">Cancel</button>
...
const ManualPaymentModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/10 backdrop-blur-sm">
      <div className="bg-card w-full max-w-5xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        <div className="px-8 py-4 flex justify-between items-start border-b border-border">
          <div>
          ...
          <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-muted-foreground"><X size={24} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
                ...
                <section className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" name="residentSearch" value={formData.residentSearch} onChange={handleChange}
                      ...
                      className="w-full pl-11 pr-10 py-3 bg-muted/50 border border-border rounded-2xl outline-none" />
                  </div>
                ...
                <section className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="relative mt-1">
                        ...
                        <input type="text" name="amountReceived" value={formData.amountReceived} onChange={handleChange}
                          className="w-full pl-8 pr-4 py-3 bg-muted/50 border border-border rounded-2xl outline-none" />
                      </div>
                    </div>
                    <div>
                      ...
                      <input type="date" name="paymentDate" value={formData.paymentDate} onChange={handleChange}
                        className="w-full mt-1 px-4 py-3 bg-muted/50 border border-border rounded-2xl outline-none" />
                    </div>
                  </div>
                  <div>
                      ...
                      {['Cash', 'Cheque', 'Transfer', 'UPI'].map((method) => (
                        <button key={method} type="button" onClick={() => setFormData(prev => ({ ...prev, paymentMethod: method }))}
                          className={`flex-1 py-2.5 px-4 rounded-2xl text-sm font-bold transition-all ${
                            formData.paymentMethod === method ? 'border border-[#3B82F6] bg-[#3B82F6]/10' : 'border border-gray-200 bg-muted/50'
                          }`}>
                          {method}
                        </button>
                      ))}
                  ...
                  <div>
                    ...
                    <textarea name="remark" value={formData.remark} onChange={handleChange}
                      ...
                      className="w-full mt-1 px-4 py-3 bg-muted/50 border border-border rounded-2xl outline-none resize-none" />
                  </div>
                  <div className="flex items-center justify-between p-4 bg-muted/50/80 rounded-2xl border border-dashed border-border">
                    ...
                    <button type="button" onClick={() => setFormData(prev => ({ ...prev, generateReceipt: !prev.generateReceipt }))}
                      ...
                      <div className={`absolute top-1 w-4 h-4 bg-card rounded-full transition-all ${formData.generateReceipt ? 'left-7' : 'left-1'}`} />
                    </button>
                  </div>
                </section>
              </div>
              ...
              <div className="w-full lg:w-72 space-y-6">
                <div className="bg-[#3B82F6] rounded-3xl p-6 text-white">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="h-px bg-card/20 w-full" />
                    <div className="flex justify-between items-start">
                      <div>
                    ...
                    <div className="flex items-center justify-center gap-2 bg-card/10 py-2.5 px-4 rounded-2xl">
                      <ShieldCheck size={16} />
          ...
          <div className="py-4 px-8 border-t border-border flex justify-end gap-4">
            <button type="button" onClick={onClose} className="px-8 py-2 text-muted-foreground font-semibold">Cancel</button>
...
const EditChargeModal = ({ isOpen, onClose, chargeId }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/10 backdrop-blur-md" onClick={onClose} />
      <div className="relative bg-card w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-foreground">Edit Charge</h2>
            <p className="text-sm text-muted-foreground">Update charge details</p>
          </div>
        ...
        <div className="mb-4">
          <label className="block text-[13px] font-bold mb-2 text-muted-foreground uppercase tracking-wider">Apply To</label>
            ...
            {[{ value: "all", label: "All Units" }, { value: "specific", label: "Specific Users" }].map((opt) => (
              <button key={opt.value} type="button"
                ...
                className={`flex-1 py-3 rounded-xl font-bold text-sm border transition-all ${
                  formData.appliedTo === opt.value ? "bg-blue-600 text-white border-blue-600" : "bg-card text-slate-600 border-border hover:bg-blue-50"
                }`}>
                {opt.label}
              </button>
            ))}
        ...
        {formData.appliedTo === "specific" && (
          <div className="mb-4 border border-border rounded-2xl p-4 bg-muted/50/30">
            <div className="relative mb-3">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input type="text" placeholder="Search resident..." value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 border border-border rounded-xl bg-card outline-none text-sm" />
            </div>
              ...
              {filteredResidents.map((r) => {
                ...
                return (
                  <div key={r._id} onClick={() => toggleResident(r._id)}
                    className={`flex justify-between items-center px-4 py-3 cursor-pointer border-b border-slate-50 last:border-0 ${isSelected ? "bg-blue-50" : "hover:bg-muted/50"}`}>
        ...
        <label className="flex items-center gap-3 w-full px-4 py-3 border border-dashed border-border rounded-xl cursor-pointer mb-6 hover:bg-muted/50">
          <Upload className="w-5 h-5 text-muted-foreground" />
          <span className="text-sm text-muted-foreground">{proofFile ? proofFile.name : "Upload new proof image (optional)"}</span>
        ...
        <div className="flex gap-4">
          ...
          <button onClick={onClose} className="flex-1 border py-3 rounded-xl font-semibold hover:bg-muted/50">Cancel</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="min-h-screen text-[#0F172A]">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {MOCK_DATA.stats.map((stat, idx) => (
              <div key={idx} className="bg-card p-6 rounded-3xl border border-[#E2E8F0] shadow-sm hover:shadow-md transition-shadow">
          ...
          {/* Quick Actions */}
          <div className="bg-card p-4 sm:px-6 sm:py-4 rounded-3xl border border-border shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 w-full md:w-auto">
              <button onClick={() => setIsGenerateBillModalOpen(true)}
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-primary text-primary-foreground px-5 py-2.5 rounded-2xl text-sm font-semibold hover:bg-primary/90 shadow-sm">
                <ScrollText size={18} /> Generate Monthly Bills
              </button>
              <button onClick={() => setIsRecordManualOpen(true)}
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 border border-border bg-card text-foreground px-5 py-2.5 rounded-2xl text-sm font-semibold hover:bg-muted transition-colors">
                <CreditCard size={18} className='text-primary' /> Record Manual Payment
              </button>
              <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 border border-border bg-card text-foreground px-5 py-2.5 rounded-2xl text-sm font-semibold hover:bg-muted transition-colors">
                <BellRing size={18} className='text-primary' /> Send Reminders
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-card rounded-3xl border border-border shadow-sm overflow-hidden">
                <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border">
                  <h3 className="text-lg font-semibold text-foreground">Maintenance Records</h3>
                  <div className="relative w-full sm:w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
                    <input type="text" placeholder="Search Unit or Resident..."
                      className="w-full bg-muted/50 border-none rounded-xl py-2 pl-10 pr-4 text-sm outline-none text-foreground" />
                  </div>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold border-b border-border bg-muted/20">
                    ...
                    <tbody className="divide-y divide-border">
                      {charges?.map((row) => (
                        <tr key={row._id} className="group hover:bg-muted/50 transition-colors">
                          <td className="px-6 py-5 font-bold text-foreground">{row.unitType || '—'}</td>
                          <td className="px-6 py-5"><p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">{row.title}</p></td>
                          <td className="px-6 py-5">
                            <span className={`font-bold ${row.status === "OVERDUE" ? "text-destructive" : "text-foreground"}`}>₹{row.amount}</span>
                          </td>
                          <td className="px-6 py-5">
                            <p className={`text-xs font-medium ${row.status === "OVERDUE" ? "text-destructive/80" : "text-muted-foreground"}`}>
              ...
              <div className="bg-card rounded-3xl border border-border shadow-sm overflow-hidden">
                <div className="p-6 flex items-center justify-between border-b border-border">
                  ...
                  <span className="text-xs text-muted-foreground font-bold uppercase">{payments?.length || 0} payments</span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                    ...
                    <tbody className="divide-y divide-border">
                      {payments?.length > 0 ? payments.map((payment) => (
                        <tr key={payment._id} className="group hover:bg-muted/50 transition-colors">
                          <td className="px-6 py-5">
                            <p className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{payment.user?.name || "—"}</p>
                            <p className="text-xs text-muted-foreground">Unit: {payment.user?.unit?.flatNumber || "—"}</p>
                          </td>
                          <td className="px-6 py-5"><span className="font-bold text-foreground">₹{payment.amount}</span></td>
                          <td className="px-6 py-5"><span className="text-sm text-muted-foreground font-medium">{payment.method || "—"}</span></td>
                          <td className="px-6 py-5">
                            <p className="text-xs font-medium text-muted-foreground">{new Date(payment.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' })}</p>
                          </td>
                          <td className="px-6 py-5 text-center">
                        ...
                        <tr>
                          <td colSpan={6} className="px-6 py-10 text-center text-muted-foreground text-sm">No payments found</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            <div className="space-y-8">
              <div className="bg-card p-6 rounded-3xl border border-gray-50 shadow-sm">
              ...
              <div className="bg-card rounded-3xl border border-gray-50 shadow-sm overflow-hidden">
        ...
        {isRejectModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-card w-full max-w-md rounded-3xl shadow-2xl p-8">

              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold text-foreground">Reject Payment</h2>
                  <p className="text-sm text-muted-foreground">Reason batao kyun reject kar rahe ho</p>
                </div>
              ...
              <div className="mb-6">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border border-border rounded-xl outline-none focus:ring-2 focus:ring-red-100 text-sm resize-none"
                />
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => { setIsRejectModalOpen(false); setRejectionReason(""); setRejectPaymentId(null); }}
                  className="flex-1 border border-border py-3 rounded-xl font-semibold hover:bg-muted/50 transition-colors">
                  Cancel
                </button>
              </div>
            </div>
