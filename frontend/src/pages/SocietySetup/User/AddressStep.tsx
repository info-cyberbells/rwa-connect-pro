...
const AddressStep = () => {
  return (
    <div className="min-h-screen bg-[#FDFDFF] relative overflow-hidden flex flex-col font-sans">
      ...
      {/* --- 1. PREMIUM NAVBAR --- */}
      <nav className="sticky top-0 z-50 bg-card/70 backdrop-blur-2xl border-b border-border/50 px-10 py-5 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3 group cursor-pointer">
          ...
          <span className="font-black text-foreground text-xl tracking-tighter ">Luxe<span className="text-blue-600 not-italic">Society</span></span>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-10">
          {[
            ...
            <a key={item.name} className={`flex items-center gap-2 font-bold text-[10px] uppercase tracking-[0.2em] transition-all cursor-pointer ${item.active ? 'text-blue-600' : 'text-slate-300 hover:text-muted-foreground'}`}>
          ...
          <div className="flex items-center gap-4">
            <HelpCircle size={20} className="text-muted-foreground cursor-pointer hover:text-blue-600 transition-colors" />
      ...
      <main className="relative z-10 w-full max-w-7xl mx-auto px-8 py-10 flex-1 flex flex-col items-center">
          ...
          {[1, 2, 3, 4].map((step) => (
            <div key={step} className="relative z-10 flex flex-col items-center gap-2">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 transition-all ${
                ...
                step === 3 ? 'bg-card border-blue-500 text-blue-600 scale-110 shadow-lg' : 
                'bg-card border-slate-50 text-slate-200'
              }`}>
                {step < 3 ? "✓" : step}
              </div>
        ...
        {/* --- 4. ADDRESS CARD --- */}
        <div className="w-full max-w-4xl bg-card rounded-[3.5rem] p-12 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.04)] border border-slate-50 relative overflow-hidden">
          
          <div className="text-center mb-10">
            <h1 className="text-4xl font-[700] text-foreground tracking-tighter mb-3 ">Society Location Details</h1>
            <p className="text-muted-foreground font-medium">Please provide the official registered address of the housing society.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
            {/* Address Line 1 */}
            <div className="space-y-2">
              <label className="text-xs font-black text-muted-foreground uppercase tracking-widest ml-1">Address Line 1</label>
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-muted/50 border border-border rounded-[1.5rem] py-4 pl-14 pr-6 focus:bg-card focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all outline-none text-slate-700 font-medium"
                />
              </div>
            </div>
            ...
            {/* Address Line 2 */}
            <div className="space-y-2">
              <label className="text-xs font-black text-muted-foreground uppercase tracking-widest ml-1">Address Line 2 (Optional)</label>
              <div className="relative group">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <select className="w-full bg-muted/50 border border-border rounded-[1.5rem] py-4 pl-14 pr-6 focus:bg-card focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all outline-none text-muted-foreground font-medium appearance-none">
            ...
            {/* State */}
            <div className="space-y-2">
              <label className="text-xs font-black text-muted-foreground uppercase tracking-widest ml-1">State</label>
              <div className="relative group">
            ...
            {/* Country */}
            <div className="space-y-2">
              <label className="text-xs font-black text-muted-foreground uppercase tracking-widest ml-1">Country</label>
              <div className="relative group">
                ...
                <select className="w-full bg-muted/50 border border-border rounded-[1.5rem] py-4 pl-14 pr-6 focus:bg-card focus:border-blue-400 focus:ring-4 focus:ring-blue-50 transition-all outline-none text-slate-700 font-medium appearance-none">
                  <option>India</option>
                </select>
                <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none border-t-4 border-x-4 border-x-transparent border-t-slate-300" />
              </div>
          ...
          <div className="mt-10 relative h-32 w-full rounded-[2rem] overflow-hidden border border-border shadow-inner group">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="px-6 py-2 bg-card/90 backdrop-blur shadow-xl rounded-full border border-white flex items-center gap-2">
                <div className="w-2 h-2 bg-blue-500 rounded-full animate-ping" />
                <span className="text-[10px] font-black text-foreground uppercase tracking-widest">Location Preview Based on Address</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* --- 5. STICKY FOOTER --- */}
      <footer className="sticky bottom-0 bg-card/80 backdrop-blur-2xl border-t border-border px-16 py-8 flex justify-between items-center z-50">
        ...
        <div className="flex items-center gap-10">
           <button className="text-muted-foreground font-black text-xs uppercase tracking-[0.2em] hover:text-slate-700 transition-colors">
            Cancel
           </button>
