      ...
      <main className="md:ml-64 w-full md:w-auto p-4 md:p-8 lg:p-12 transition-all duration-300">
        <div className="max-w-4xl mx-auto">
          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pt-12 md:pt-0">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-foreground block">
            ...
            <div className="flex gap-4 md:gap-6 w-full sm:w-auto justify-between sm:justify-end">
              <button className="flex items-center gap-1.5 text-xs md:text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors whitespace-nowrap">
          ...
          <div className="bg-card rounded-2xl md:rounded-3xl border border-border shadow-sm overflow-hidden">
            {/* Tabs */}
            <div className="flex border-b border-border px-4 md:px-6 overflow-x-auto no-scrollbar bg-card sticky top-0 z-10">
              {['All Notifications', 'System Alerts', 'Society Requests', 'Payments'].map((tab) => (
                <button
                  ...
                  className={`py-4 md:py-5 px-3 md:px-4 text-xs md:text-sm font-semibold transition-all border-b-2 whitespace-nowrap -mb-[1px] ${
                    activeTab === tab ? 'border-blue-600 text-blue-600' : 'border-transparent text-muted-foreground hover:text-slate-600'
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="divide-y divide-slate-50">
              {notifications.map((note, idx) => (
                <div key={idx} className="p-4 md:p-6 flex flex-col sm:flex-row gap-3 md:gap-4 hover:bg-muted/50 transition-colors cursor-pointer">
                  <div className="flex items-start justify-between sm:block">
                    ...
                    <span className="text-[10px] text-muted-foreground sm:hidden">{note.time}</span>
                  </div>

                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold text-foreground text-sm md:text-base">{note.title}</h3>
                      <span className="hidden sm:block text-xs text-muted-foreground whitespace-nowrap ml-2">{note.time}</span>
                    </div>
            ...
            <div className="p-4 text-center border-t border-slate-50 bg-muted/30">
