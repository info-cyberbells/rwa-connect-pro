  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8] flex items-center justify-center p-6 font-sans">
      <motion.div 
        ...
        className="w-full max-w-5xl bg-card/80 backdrop-blur-md rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white overflow-hidden"
      >
        
        {/* --- Header / Navigation --- */}
        <header className="px-10 py-8 border-b border-dashed border-blue-100 bg-card/50">
          <div className="flex justify-between items-center mb-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-black text-foreground text-xl tracking-tight">Society Identity</h1>
                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Management Suite</p>
              </div>
            </div>
          ...
          <div className="flex justify-between relative max-w-3xl mx-auto">
            {steps.map((step, idx) => (
              <div key={step.id} className="flex flex-col items-center z-10 group cursor-pointer">
                ...
                <span className={`text-[11px] font-black uppercase tracking-wider ${step.status === 'active' ? 'text-blue-600' : 'text-muted-foreground'}`}>
                  {step.label}
                </span>
              </div>
            ))}
            <div className="absolute top-7 left-0 w-full h-[3px] bg-muted/50 -z-0 rounded-full" />
        ...
        <main className="p-12 grid grid-cols-1 lg:grid-cols-2 gap-20">
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="group">
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2 group-focus-within:text-blue-600 transition-colors">
                    {field.label}
                  </label>
                  <input 
                    type={field.type}
                    placeholder={field.placeholder}
                    className="w-full p-4 bg-muted/50/50 border border-border rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:bg-card outline-none transition-all duration-300 placeholder:text-slate-300 font-medium text-slate-700"
                  />
                </div>
              ))}
            </div>
          ...
          <section className="flex flex-col items-center">
            <div className="w-full space-y-4">
               <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest text-center lg:text-left">Society Logo</label>
               <div className="w-full aspect-square max-w-[340px] mx-auto border-2 border-dashed border-blue-100 rounded-[3rem] flex flex-col items-center justify-center p-10 bg-blue-50/20 group cursor-pointer hover:bg-card hover:border-blue-400 hover:shadow-2xl hover:shadow-blue-100 transition-all duration-500">
                  <div className="relative mb-6">
                    <div className="w-24 h-24 bg-card shadow-[0_20px_40px_rgba(0,0,0,0.08)] rounded-3xl flex items-center justify-center border border-slate-50 group-hover:scale-110 transition-transform duration-500">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1 text-center font-medium leading-relaxed">
        ...
        {/* --- Footer / Navigation --- */}
        <footer className="px-10 py-8 border-t border-dashed border-blue-100 bg-muted/50/50 flex items-center justify-between">
          <button className="text-muted-foreground font-black text-xs uppercase tracking-widest hover:text-red-500 transition-colors">
            Cancel Process
          </button>
          ...
          <div className="flex items-center gap-10">
            <div className="text-right hidden sm:block">
              ...
              <div className="flex items-center gap-3">
                <span className="text-lg font-black text-foreground">25%</span>
