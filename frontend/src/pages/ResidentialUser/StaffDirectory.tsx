      ...
      <div className="max-w-[1400px] mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Trusted Staff Directory</h1>
            <p className="text-slate-500 font-medium max-w-md">Find and rate service providers verified by your society management.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-12 pr-6 py-3 bg-white border border-slate-200 rounded-2xl w-full sm:w-72 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-50/50 transition-all shadow-sm font-medium"
                value={searchTerm}
            ...
            <div className="relative w-full sm:w-48" ref={dropdownRef}>
              <div 
                ...
                className={`flex items-center justify-between px-5 py-3 bg-white border rounded-2xl cursor-pointer transition-all shadow-sm hover:border-blue-300 ${isRoleDropdownOpen ? 'border-blue-500 ring-4 ring-blue-50' : 'border-slate-200'}`}
              >
                <div className="flex items-center gap-2">
                  <Filter size={16} className={selectedRole !== "All" ? "text-blue-600" : "text-slate-400"} />
                  <span className={`text-sm font-bold ${selectedRole !== "All" ? "text-slate-800" : "text-slate-500"}`}>{selectedRole}</span>
                </div>
                      ...
                      {roles.map((role) => {
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-center justify-between px-4 py-2.5 rounded-xl cursor-pointer transition-all ${isSelected ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50 text-slate-600'}`}
                          >
        ...
        {loading && staffData.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredStaff.map((staff: any) => (
              <motion.div 
                layout
                key={staff._id}
                className="bg-white rounded-[28px] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 hover:-translate-y-1 transition-all group relative overflow-hidden"
              >
                {/* Status Indicator */}
                <div className="absolute top-4 right-4 z-10">
                   <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/90 backdrop-blur-sm rounded-full border border-slate-100 shadow-sm">
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-col items-center text-center mb-6">
                    <div className="relative mb-4 group-hover:scale-105 transition-transform duration-300">
                      <div className="w-20 h-20 rounded-[28px] bg-slate-100 flex items-center justify-center text-3xl font-black text-slate-300 overflow-hidden bg-cover bg-center border-4 border-white shadow-lg ring-1 ring-slate-100"
                           style={{ backgroundImage: staff.photo ? `url(${staff.photo})` : 'none' }}>
                        {!staff.photo && (staff.staffName ? staff.staffName[0] : "?")}
                      </div>
                      <div className="absolute -bottom-1 -right-1 bg-blue-600 text-white p-1.5 rounded-xl border-2 border-white shadow-md">
                        <ShieldCheck size={14} />
                      </div>
                    </div>

                    <h3 className="text-lg font-bold text-slate-800 leading-tight">{staff.staffName}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[11px] font-bold text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded-full uppercase tracking-wider border border-blue-100/50">
                        {staff.role}
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Rating Stats */}
                  <div className="flex items-center justify-center gap-6 py-4 border-y border-slate-50 mb-6 bg-slate-50/50 rounded-2xl">
                    <div className="text-center">
                      <div className="flex items-center gap-1 text-amber-500 mb-0.5">
                        <Star size={16} fill="currentColor" />
                        <span className="font-bold text-slate-800">{staff.averageRating || "0.0"}</span>
                      </div>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Avg Rating</p>
                    </div>
                    <div className="w-px h-8 bg-slate-200" />
                    <div className="text-center">
                      <span className="font-bold text-slate-800 block mb-0.5">{staff.totalReviews || 0}</span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      ...
                      className="py-3 rounded-2xl bg-blue-600 text-white text-[11px] font-bold flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95 shadow-lg shadow-blue-100"
                    >
                      <Star size={14} /> Rate
                    </button>
                    <button 
                      onClick={() => openReviews(staff)}
                      className="py-3 rounded-2xl bg-white border border-slate-200 text-slate-600 text-[11px] font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-all active:scale-95 hover:border-blue-200 hover:text-blue-600"
                    >
          ...
          <div className="py-40 flex flex-col items-center justify-center text-slate-300 gap-4">
            ...
            <p className="font-bold uppercase tracking-widest text-xs text-slate-400">No matching helpers found</p>
              ...
              >
                <div className="p-8 pb-4 flex items-center justify-between border-b border-slate-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-slate-800 tracking-tight">Rate Provider</h3>
                      <p className="text-xs text-slate-500 font-medium">Reviewing {showRatingModal.staffName}</p>
                    </div>
                  </div>
                  <button onClick={() => setShowRatingModal(null)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-400">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="p-8 space-y-8">
                  {/* Star Picker */}
                  <div className="flex flex-col items-center gap-4 py-8 bg-slate-50 rounded-[28px] border border-slate-100">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm min-h-[120px] outline-none focus:border-blue-500 focus:bg-white transition-all resize-none font-medium placeholder:text-slate-400"
                      value={userReview}
          ...
          {showReviewsModal && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                className="relative bg-white h-full max-h-[600px] w-full max-w-md shadow-2xl overflow-hidden flex flex-col rounded-[32px]"
              >
                <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-slate-800 tracking-tight">Community Reviews</h3>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{showReviewsModal.staffName}</p>
                    </div>
                  </div>
                  <button onClick={() => setShowReviewsModal(null)} className="p-2 hover:bg-white rounded-xl transition-colors text-slate-400 border border-transparent hover:border-slate-100">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {ratingLoading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-3 text-blue-600">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Fetching reviews...</span>
                    </div>
                  ) : staffReviews.length > 0 ? (
                    staffReviews.map((rev: any) => (
                      <div key={rev._id} className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm hover:border-blue-100 transition-colors">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">{new Date(rev.createdAt).toLocaleDateString()}</p>
                            </div>
                          </div>
